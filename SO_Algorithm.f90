...
integer :: i,j,k,ts,dir,cnt,rnd,fnd !Counters ts - time counter;
...
ui = 0.
open (15,file='velini_600.dat',form='formatted',status='unknown')
do j=1,nyi
    do k=1,nzi
...
dt = 0.15 !Time step for each observation
...
eps = 0.5 !Fixing value to ensure no drastic reduction
if (mod(cnt,5000) .eq. 0) eps = 1 !Initial high value to enable mobility
...
!        t_corr(k,:) = 0
!    endif
!    if (chkerr(k) .lt. minerr(k)+2.) then
!        err = 0
!        do i = 1,num(k)
...
if (cnt .eq. cmax+1) then
    ...
    do k = 1,nzi
        ...
        print*, k,tstart(k),minerr(k),tout(k)!,num(k)
    enddo
    call sleep(2)
endif
...
OPEN(11,FILE='ux_rec',FORM='UNFORMATTED',&
...
ENDDO
CLOSE(11)
OPEN(11,FILE='uy_rec',FORM='UNFORMATTED',&
ACCESS='DIRECT', RECL=8)
COUNT = 1
DO K=1,nzi
DO J=1,nyi
...
ENDDO
CLOSE(11)
OPEN(11,FILE='uz_rec',FORM='UNFORMATTED',&
ACCESS='DIRECT', RECL=8)
COUNT = 1
DO K=1,nzi
DO J=1,nyi
